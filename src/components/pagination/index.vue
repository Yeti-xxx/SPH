<template>
    <!-- 分页器组件 -->
    <div class="pagination">
        <button :disabled="pageNo === 1" @click="$emit('getPageNo', pageNo - 1)">上一页</button>
        <blok v-if="StartNumAndEndNum.start > 1 + 1">
            <button @click="$emit('getPageNo', 1)" :class="{ active: pageNo == 1 }">1</button>

            <button>···</button>
        </blok>

        <!-- 组件结构 -->
        <blok v-for="(page, i) in StartNumAndEndNum.end" :key="i">
            <button v-if="page >= StartNumAndEndNum.start" @click="$emit('getPageNo', page)"
                :class="{ active: pageNo === page }">{{ page }}</button>
        </blok>

        <blok v-if="StartNumAndEndNum.end < totalPage - 1">
            <button>···</button>
            <button @click="$emit('getPageNo', totalPage)" :class="{ active: pageNo == totalPage }">{{ totalPage
            }}</button>
        </blok>
        <button :disabled="pageNo == totalPage" @click="$emit('getPageNo', pageNo + 1)">下一页</button>
        <button style="margin-left: 30px">共{{ total }}条</button>
    </div>
</template>

<script>
export default {
    name: 'Pagination',
    props: ['pageNo', 'pageSize', "total", "continues"],
    computed: {
        // 页数
        totalPage() {
            return Math.ceil(Number(this.total) / Number(this.pageSize))
        },
        StartNumAndEndNum() {
            // 存储分页器起始数字和结束数字
            let start = 0, end = 0
            // 总页数小于连续页码
            if (this.continues > this.totalPage) {
                start = 1
                end = this.totalPage
            } else {
                start = this.pageNo - parseInt(this.continues / 2)
                end = this.pageNo + parseInt(this.continues / 2)
                if (start < 1) {    //如果start页码算出小于1,即 pageNo < parseInt(this.continues / 2)
                    start = 1   //进行纠正
                    end = this.continues
                } else if (end > this.totalPage) {
                    start = this.totalPage - this.continues
                    end = this.totalPage

                }
            }
            return { start, end }
        }
    }

}

</script>

<style lang='less' scoped>
.pagination {
    text-align: center;

    button {
        margin: 0 5px;
        background-color: #f4f4f5;
        color: #606266;
        outline: none;
        border-radius: 2px;
        padding: 0 4px;
        vertical-align: top;
        display: inline-block;
        font-size: 13px;
        min-width: 35.5px;
        height: 28px;
        line-height: 28px;
        cursor: pointer;
        box-sizing: border-box;
        text-align: center;
        border: 0;

        &[disabled] {
            color: #c0c4cc;
            cursor: not-allowed;
        }

        &.active {
            cursor: not-allowed;
            background-color: #e1251b;
            color: #fff;
        }
    }
}
</style>

